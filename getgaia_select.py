...
def main(argv):  
   ...
   if args.use_members:
      ...
      probabilities = CalculateProbabilities(args, data.copy(), field.copy(),
                                             systematics = pd.DataFrame(data={'pmra_error': 0.015, 'pmdec_error': 0.015, 'parallax_error': 0.007, 'gmag_error': 0.01, 'bp_rp_error': 0.01}, index =[0]), probs_filename = "./%s/%s_probabilities.csv"%(args.name, args.name), ellipse_filename="./%s/%s_ellipse.csv"%(args.name, args.name))
