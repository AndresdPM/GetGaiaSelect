...
def sky_model(args, data, field, probabilities):
   ...
   ngauss = 900 # Number of gaussians for the fitting.
...
def CalculateProbabilities(args, data, field, systematics = None):
   """
   ...
   probabilities.to_csv('test_prob.csv', index = False)
   return probabilities
...
def manual_select_from_cmd(args, data):
   ...
   plt.subplots_adjust(right=0.85, wspace = 0.3, bottom=0.15)
      ...
      try:
         ...
         if any((args.min_pmra == None, args.max_pmra == None)):
            if (object_table['PMRA'].mask == False):
               ...
               args.max_pmra = float(object_table['PMRA']) + 2
               args.min_pmra = float(object_table['PMRA']) - 2
            else:
               if not args.silent:
                  try:
                     args.max_pmra = float(input('Max PMRA not defined, please enter pmra in mas/yr (Press enter to adopt the default value of 2 m.a.s./yr): '))
                  except:
                     args.max_pmra = 2.0
                  try:
                     args.min_pmra = float(input('Min PMRA not defined, please enter pmra in mas/yr (Press enter to adopt the default value of  -2 m.a.s./yr): '))
                  except:
                     args.min_pmra = -2.0
         ...
         if any((args.min_pmdec == None, args.max_pmdec == None)):
            if (object_table['PMDEC'].mask == False):
               ...
               if not args.silent:
                  try:
                     args.max_pmdec = float(input('Max PMDEC not defined, please enter pmdec in mas/yr (Press enter to adopt the default value of 2 m.a.s./yr): '))
                  except:
                     args.max_pmdec = 2.0
                  try:
                     args.min_pmdec = float(input('Min PMDEC not defined, please enter pmdec in mas/yr (Press enter to adopt the default value of  -2 m.a.s./yr): '))
                  except:
                     args.min_pmdec = -2.0
         ...
         if (args.max_parallax is None) and not args.silent:
            try:
               args.max_parallax = float(input('Max parallax not defined, please enter parallax in mas/yr (Press enter to adopt the default value of 2 m.a.s.): '))
            except:
               args.max_parallax = 2.0
...
def main(argv):  
   ...
   parser.add_argument('--clipping_prob_pm', type=float, default=3., help='Sigma used for clipping pm and parallax. Default is 4.')
   ...
   parser.add_argument('--inside_core', type = str2bool, default = True, help='Use only stars inside the core of the galaxy. Default is True.')
   ...
   if args.use_members:
      ...
      probabilities = CalculateProbabilities(args, data.copy(), field.copy(),
                                             systematics = pd.DataFrame(data={'pmra': 0.035, 'pmdec': 0.035, 'parallax': 0.015, 'gmag': 0., 'bp_rp': 0.}, index =[0]))

      data.loc[:, 'membership_prob'] = probabilities.loc[:, 'membership_prob']
   ...
   #TODO: - Con find galaxy, puedes imponer una elipse dentro de la cual tienen que estar las estrellas. O tal vez no usar la ultima gausiana.
         #- Poner los sistematicos, y ya esta!
         #- Computar las velocidades, aunque eso es un pego.
